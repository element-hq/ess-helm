        ...
        for deployable_details in all_deployables_details:
            ...
            if deployable_details.has_exposed_services:
                assert len(found_certificates[deployable_details.name]) == len(
                    ...
                    f"{len(found_exposed_services_using_cert_manager[deployable_details.name])} services using cert-manager: "
        ...
        if template["kind"] == "Service":
            ...
            for port in template["spec"]["ports"]:
                assert int(port["port"]), (
                    f"{template_id(template)} port {port.get('name', 'unnamed')} has invalid port number: {port['port']}. "
                    f"Port must be a valid integer."
                )
                if port.get("nodePort"):
                    assert int(port["nodePort"]), (
                        f"{template_id(template)} port {port.get('name', 'unnamed')} has invalid nodePort number: {port['nodePort']}. "
    ...
    for template in templates:
        if template["kind"] == "Service":
            ...
            assert number_of_ports > 0, (
                f"{template_id(template)} has no ports defined. "
                f"Services must specify at least one port."
            )
        ...
        for line in template_as_yaml.splitlines():
            if ".svc" in line:
                assert ".svc.k8s.example.com." in line, (
                    f"{template_id(template)} contains service reference that doesn't use configured cluster domain: {line=}. "
    ...
    for template in templates:
        if template["kind"] == "Service":
            assert template["spec"]["ipFamilyPolicy"] == "PreferDualStack", (
                f"{template_id(template)} has ipFamilyPolicy '{template['spec']['ipFamilyPolicy']}' instead of 'PreferDualStack'. "
