...
towncrier check  --compare-with="origin/main" || towncrier check --dir packages/ess-migration-tool --compare-with="origin/main" || (echo -e "$CONTRIBUTING_GUIDE_TEXT" >&2 && exit 1)
...
matched=0
for f in $(git diff --diff-filter=d --name-only origin/main... -- ':(exclude)newsfragments/.gitkeep'  newsfragments packages/ess-migration-tool/newsfragments); do
    ...
    [[ -n "$pr" && ( "$f" == newsfragments/"$pr".* || "$f" == packages/ess-migration-tool/newsfragments/"$pr".* ) ]] && matched=1
done
